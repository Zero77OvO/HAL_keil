/***********************************
	本驱动文件仅适配HAL库版本
***********************************/
#include "stm32f4xx_hal.h"	//链接HAL库，你的是什么芯片就改成什么
...
/*
   OLED显示数字（十进制，带符号数）
   X,Y为起始坐标点
    Num 要显示的数字，范围：-2147483648~2147483647
    Len 要显示数字的长度，范围：1~10
*/
...
//显示一个字符号串
//x,y为坐标，chr为字符串，Char_Size为单个字母或符号占用的像素长度
...
//显示128x64 BMP图片
void OLED_Draw12864BMP(uint8_t num)//其中的变量为第几张图片
{
    uint16_t j=0;
    uint8_t x,y;
    for(y=0; y<8; y++)
